...
namespace VTabs
{
    public static class VTabs
    {
        ...
        static IEnumerable<EditorWindow> allPropertyEditors => _allPropertyEditors ??= t_PropertyEditor.GetFieldValue<IList>("m_AllPropertyEditors").Cast<EditorWindow>().Where(r => r.GetType().BaseType == typeof(EditorWindow));
        static IEnumerable<EditorWindow> _allPropertyEditors;
#else
...
#if UNITY_2023_1_OR_NEWER
        static List<EditorWindow> allEditorWindows => _allEditorWindows ??= t_EditorWindow.GetMemberValue<List<EditorWindow>>("activeEditorWindows");
        static List<EditorWindow> _allEditorWindows;
#else
